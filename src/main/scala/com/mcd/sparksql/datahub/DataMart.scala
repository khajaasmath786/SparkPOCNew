...
object DataMart{
   ...
   if (args.length < 0) {
    ...
    val conf = new SparkConf().setAppName("SparkProps").setMaster(args(0))
    ...
    val master=sparkContext.getConf.get("Master")
    println(master)
    //val driverMemory=sparkContext.getConf.get("Driver.Memory")
    //val executorMemory=sparkContext.getConf.get("Executor.Memory")
    val jobName="DataMartRefreshProcess"
    
    
    
    val refreshDays=sparkContext.getConf.get( "REFRESH_DAYS")
    
    val DLY_DYPT_POS_TRN_NP_POPULATE=sparkContext.getConf.get( "DLY_DYPT_POS_TRN_NP_POPULATE")
    val DLY_TM_SEG_POS_TRN_OFFR_NP_POPULATE=sparkContext.getConf.get( "DLY_TM_SEG_POS_TRN_OFFR_NP_POPULATE")
    val POS_TRN_OFFR_NP_POPULATE=sparkContext.getConf.get("POS_TRN_OFFR_NP_POPULATE")
